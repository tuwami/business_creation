class Investment < ApplicationRecord
  ...
  def cal_params_market_recruiting
    if market.market_master_id % 18 == 0
  ...
  def cal_params_balance
    if market.market_master_id % 18 == 0
      ...
      (1..17).each do |num|
        if num == market.market_master_id % 18
          return cal_balance(market.market_master.sales_retention_index).to_f
        end
      end
    end
  end
  ...
  def cal_balance(a)
    return (( market.balance + market.market_budget_earning ) * a ).to_s.to_d.floor(2).to_f
  end
end
